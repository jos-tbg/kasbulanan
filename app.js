...
document.addEventListener("DOMContentLoaded", function() {
  ...
  penyesuaianFormKategori();  // 2. BARU SET NILAI DEFAULT & PILIH BULAN AUTOMATIS
...
function cekModeInputBulanForm() {
  ...
  const skrg = new Date();
  const bulanSekarangIdx = skrg.getMonth(); // 0 = Jan, 11 = Des

  // KATEGORI DONASI / LAIN-LAIN (DONATUR)
  if (kategori === "Donasi" || kategori === "Lainnya") {
    containerChips.classList.add("hidden");
    containerDropdown.classList.remove("hidden");
    
    // Otomatis Bulan Sekarang
    selectBulan.value = DAFTAR_BULAN[bulanSekarangIdx];
    pilihanBulanForm = [];
    return;
  }

  // KATEGORI ANGGOTA (KAS BULANAN / DANA SOSIAL)
  if (nominal > 100000 && nominal % 100000 === 0) {
    // Kelipatan 100rb -> Multi-select Chips (Tanpa Pilihan Otomatis)
    ...
    if (nominal === 100000) {
      // Jika 100.000 -> Otomatis Bulan Sekarang - 1 (Bulan Lalu)
      let idxBulanLalu = bulanSekarangIdx - 1;
      if (idxBulanLalu < 0) idxBulanLalu = 11; // Jika Januari, mundur ke Desember
...
function penyesuaianFormKategori() {
  ...
  pilihanBulanForm = []; // Reset chips/multi-select
  ...
  if (kategori === "Donasi" || kategori === "Lainnya") {
    ...
    // Nominal default 100.000 untuk anggota
    inputNominal.value = new Intl.NumberFormat('id-ID').format(100000);
  }

  // Jalankan pengecekan mode & isi bulan secara otomatis
  cekModeInputBulanForm();
}
...
function bukaPreviewDetailIuran(data) {
  ...
  if (data.bukti && data.bukti !== "-" && String(data.bukti).trim() !== "") {
    let rawUrl = String(data.bukti).trim();

    if (rawUrl.includes("drive.google.com")) {
      const match = rawUrl.match(/\/d\/([a-zA-Z0-9_-]+)/) || rawUrl.match(/id=([a-zA-Z0-9_-]+)/);
      if (match && match[1]) {
        rawUrl = "https://lh3.googleusercontent.com/d/" + match[1];
      }
    }
      ...
      container.innerHTML += `
        <div class="bg-amber-50/60 border border-amber-200 rounded-2xl p-3.5 flex flex-col sm:flex-row items-center justify-between gap-3 shadow-sm">
          <div class="flex items-center gap-3 w-full sm:w-auto">
            <div class="w-16 h-16 bg-white border border-amber-200 rounded-xl overflow-hidden shrink-0 flex items-center justify-center cursor-pointer" onclick="bukaModal('${item.bukti}')">
              ${item.bukti !== "-" 
                ? `<img src="${item.bukti}" class="w-full h-full object-cover hover:scale-105 transition" alt="Bukti">` 
          ...
          <div class="flex items-center gap-2 w-full sm:w-auto border-t sm:border-t-0 pt-2 sm:pt-0 border-amber-200 justify-end">
            ${item.bukti !== "-" ? `<button onclick="bukaModal('${item.bukti}')" class="px-3 py-1.5 bg-white border border-slate-200 text-slate-600 rounded-xl text-xs font-bold hover:bg-slate-50 flex items-center gap-1"><i class="fa-solid fa-eye"></i> Perbesar Foto</button>` : ''}
...
function renderAdminPending(pendingData) {
  const tbodyPending = document.getElementById("tabelPending");
  ...
  if (pendingData.length > 0) {
    ...
    pendingData.forEach(item => {
      ...
      tbodyPending.innerHTML += `
        <tr>
          ...
          <td class="px-3 py-2 text-[11px] text-slate-500">${periodeStr}</td>
          <td class="px-3 py-2 font-bold text-emerald-600">${formatRupiah(item.nominal)}</td>
          <td class="px-3 py-2">
            ${item.bukti !== "-" ? `<button onclick="bukaModal('${item.bukti}')" class="text-[11px] bg-indigo-50 text-indigo-600 px-2 py-0.5 rounded font-semibold">Foto</button>` : '-'}
          </td>
          <td class="px-3 py-2 text-center space-x-1">
            <button onclick="bukaModalPin(${item.rowId}, 'Disetujui')" class="bg-emerald-500 text-white text-[10px] px-2 py-1 rounded font-bold hover:bg-emerald-600">Setujui</button>
            <button onclick="bukaModalPin(${item.rowId}, 'Ditolak')" class="bg-rose-500 text-white text-[10px] px-2 py-1 rounded font-bold hover:bg-rose-600">Tolak</button>
          </td>
        </tr>
      `;
    });
    ...
    tbodyPending.innerHTML = `<tr><td colspan="6" class="text-center py-5 text-slate-400">Tidak ada transaksi pending.</td></tr>`;
  }
}
